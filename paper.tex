...
\date{Preprint v1.0.0 --- 9 September 2026\\Paper DOI: \href{https://doi.org/10.5281/zenodo.22803251}{10.5281/zenodo.22803251}\\Software DOI: \href{https://doi.org/10.5281/zenodo.22803253}{10.5281/zenodo.22803253}}
...
\]
The $q_1$ branch cannot be minimizing, since it would give
$D(S)=1+D(q_1)=2+D(S)$. Thus
...
\]
By monotonicity, $D(\{q_1,q_{n-2}\})\ge D(q_1)=D(S)+1$, so this branch also cannot minimize in view of \eqref{eq:S-R}. Hence
\begin{equation}\label{eq:R-P}
D(R)=1+D(P_{n-2}).
\end{equation}
For $5\le i\le n-2$, both inverse letters map $P_i$ to $P_{i-1}$, and therefore
\begin{equation}\label{eq:Pshift}
D(P_{n-2})=(n-6)+D(P_4).
\end{equation}
It remains to compute $D(P_4)$. We have
\[
a^{-1}(P_4)=\{q_4,q_n\}=:X,\qquad
b^{-1}(P_4)=\{q_3,q_4\}=:Y.
\]
For $Y$,
\[
a^{-1}(Y)=\{q_n\},\qquad b^{-1}(Y)=S.
\]
Lemma~\ref{lem:singletons} and monotonicity give
$D(q_n)=n-2$ and $D(S)\ge D(q_2)=n-2$, hence
\[
D(Y)=n-1.
\]
For $X$,
\[
a^{-1}(X)=\{q_{n-1},q_n\},\qquad
b^{-1}(X)=\{q_3,q_n\}=:Z.
\]
Monotonicity gives $D(\{q_{n-1},q_n\})\ge D(q_n)=n-2$. Also
$a^{-1}(Z)=\{q_{n-1}\}$, so
\[
D(Z)\le1+D(q_{n-1})=n-2,
\]
while monotonicity gives the reverse inequality $D(Z)\ge D(q_n)=n-2$. Thus $D(Z)=n-2$ and $D(X)=n-1$. Consequently
\[
D(P_4)=1+\min\{D(X),D(Y)\}=n.
\]
Combining \eqref{eq:S-R}, \eqref{eq:R-P}, and \eqref{eq:Pshift} yields
\[
D(S)=2+(n-6)+n=2n-4.
\]
\end{proof}
...
For every $n\ge6$, the binary automaton $T_n$ is strongly connected and synchronizing, and
\[
\operatorname{at}_1(T_n)=2n-3.
\]
More precisely,
\[
\bigl(D(q_1),D(q_2),\ldots,D(q_n)\bigr)
=\bigl(2n-3,n-2,1,2,\ldots,n-2\bigr).
\]
\end{theorem}
\begin{proof}
Strong connectivity and synchronization were proved above. By Lemma~\ref{lem:pair},
\[
D(q_1)=1+D(\{q_2,q_3\})=2n-3.
\]
Lemma~\ref{lem:singletons} gives all remaining entries of the vector, whose largest value is $2n-3$.
\end{proof}

An explicit shortest word avoiding $q_1$ is
\[
 a b a^{n-4} b a^{n-5} b^2 a,
\]
of length $2n-3$.
...
\paragraph{Archival identifiers and artifacts.} The publication record for this preprint is DOI \href{https://doi.org/10.5281/zenodo.22803251}{10.5281/zenodo.22803251}. The frozen source and reproducibility package is archived separately as DOI \href{https://doi.org/10.5281/zenodo.22803253}{10.5281/zenodo.22803253}. The public source repository is \url{https://github.com/yonezaemon1-hub/local-rigidity-twelve-states-1-avoiding-threshold}.
